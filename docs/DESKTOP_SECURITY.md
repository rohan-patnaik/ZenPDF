...
The local state database contains recent paths and timestamps, plus future index, recovery, and audit metadata. Window preferences are stored separately as described below. Logs contain application events and error categories; document bytes, extracted text, form values, passwords, cryptographic keys, and full user paths must not be logged by default. Users must be able to clear recent history and derived indexes without touching their documents.
...
Window geometry and main-window state use an explicit `preferences.ini` leaf in that same application-data directory. QSettings parses and serializes only unique private staging files; encoded output is measured and the QSaveFile temporary descriptor's owner, type, mode, link count, and size are verified before atomic commit, so a reported publication failure leaves the prior snapshot unchanged. The platform-independent 96 KiB total and 32 KiB per decoded-value limits cannot replace a prior valid snapshot with oversized escaped output. No-follow classification rejects aliases, including dangling links, before every final-path read or publication. The final leaf requires an owner-owned, single-link regular file, rejects special or group/other-writable modes, and is repaired to 0600 only from a non-writable safe mode. Schema 1 accepts bounded byte-array values. The legacy parent is opened no-follow and must remain the same effective-UID-owned directory without group/other write or special bits. When that parent exists, the one-time versionless import acquires a transient, validated QLockFile lease before rechecking whether the legacy final exists and holds it from the stable snapshot through current publication. This does not create a legacy coordinator or change the legacy parent, mode, bytes, timestamps, or inventory; an absent legacy parent is not created, and import publishes only if the current leaf is still absent while holding the current-path lease. A save with no current snapshot uses the same legacy-then-current lock order: active, unsafe, corrupt, future, or otherwise unresolved legacy state fails explicitly, while a valid legacy snapshot must be loaded/imported before a new current save can replace it.

On Unix, cooperating instances serialize read, migration, and publication with a validated 0600 lock leaf opened nonblocking and a 100 ms monotonic nonblocking deadline. The former QSettings final-path lock is also acquired without waiting, but only after any existing sidecar passes no-follow owner/type/link/mode/size/content checks; unsafe crash debris is preserved and rejected, while a recognizable abandoned QLockFile can be recovered by Qt. Either lock's contention returns a bounded path-free failure while preserving the prior bytes. Malformed, truncated, wrong-type, oversized, invalid-restoration, and future-schema data falls back to captured defaults; a recognizable future schema is preserved even when later INI content is corrupt, and saves never replace an unparsable existing snapshot. Accepted close synchronizes and verifies storage; a canceled dirty close does not save, and any save failure requires an explicit Cancel or Close-without-saving choice rather than silently accepting close. These checks share the same effective-UID, privileged-process, kernel, filesystem, and post-validation race exclusions as the SQLite leaf.

The secured 0700 leaf excludes replacement by a different unprivileged local user. It does not defend against the same effective UID, privileged processes, a hostile kernel or filesystem, or paths disclosed before repair. On Unix, diagnostic-log setup and each write/rotation try an advisory lock on the validated directory descriptor under a 100 ms monotonic deadline; lock contention skips that file operation while sanitized stderr continues, and a setup timeout retains the descriptor so later events can recover. Each acquired event opens, validates, writes, and closes the active file under the lock so cooperating ZenPDF instances share one rotation set without retaining unlinked descriptors. Log children refuse aliases, non-regular entries, multiple links, foreign ownership, and modes other than 0600. Existing valid active and rotated logs are removed before use when oversized. The requested per-file limit is clamped to 16 KiB-1 MiB. Runtime messages are reduced to product-owned event categories before both stderr and file output; the sanitizing handler remains active if private file setup fails and until application-owned objects are destroyed. Non-Unix file handling is unchanged, while the message sanitization applies on every platform.
