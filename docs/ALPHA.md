...
ZenPDF Desktop alpha is an offline, account-free native Arch/Omarchy workspace. It is not an Acrobat compatibility claim. The capability matrix remains authoritative; L001 native Arch/Wayland launch and L016 private recent-files history and purge are `Verified`, while every other desktop row remains `Partial` or `Not started`.
