...
| L005 | Private bounded local diagnostics | M0 / Operations | Filesystem policy | Partial | `LoggingTest.cpp` covers private creation, the 1 MiB ceiling, active/rotated bounds including pre-existing oversize, hostile child rejection, serialized multi-process retention with no open-unlinked log descriptors, sanitized stderr/file/destructor output, setup and rotation failure, and bounded fatal/restart subprocess behavior; exact-package installed hostile-state and crash/privacy acceptance remains | N/A |
