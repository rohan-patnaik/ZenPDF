...
| L004 | Persistent local preferences | M0 / State | QSettings | Partial | `PreferencesTest.cpp` and `MainWindowTest.cpp` cover immutable legacy import and absent-current saves under one legacy-then-current lock order, active/missing/stale/hostile/corrupt/future legacy states without shadowing, stable no-follow legacy-parent validation, atomic save-if-absent, precommit descriptor-fault rollback, current/future and future-plus-corrupt schemas, encoded and decoded bounds, corrupt/truncated/wrong-type fallback, hostile/dangling leaf and current/legacy QLockFile-sidecar rejection/recovery, bounded lock contention, concurrent complete snapshots, save/reopen, actual/default restoration, canceled close, and explicit save-failure close policy; exact-package installed migration/relaunch plus save-failure dialog role/name evidence remains | N/A |
| L005 | Private bounded local diagnostics | M0 / Operations | Filesystem policy | Verified | `LoggingTest.cpp` covers private creation, the 1 MiB ceiling, active/rotated bounds including pre-existing oversize, hostile child rejection, serialized multi-process retention, sanitized stderr/file/destructor output, contention, failure, and bounded fatal/restart behavior; [`L005_ACCEPTANCE_EVIDENCE.md`](L005_ACCEPTANCE_EVIDENCE.md) reconciles exact-package installed identity, all 34 hostile/privacy/bounds/concurrency/crash cases, rollback, and independent acceptance | N/A |
...
| L012 | Full-document text search | M1 / Reader | Text adapter | Partial | Generated regressions plus the hashed opt-in `SearchCorpusProbe.cpp` gate cover encrypted search, independent Unicode/no-text, malformed recovery, query replacement/destruction, keyboard traversal, an 80-page fixture, source hashes, and sampled hostile/long/flood observations; hard resource limits, true cancellation/timeout, highlight fidelity, installed AT, and broad producers remain | Tagged, untagged, and broader independent-producer corpus missing |
...
| L039 | Complete keyboard and screen-reader operation | M2 / Accessibility | AT test environment | Not started | Search Tab/Shift+Tab/Down/Enter focus, role, name, status, clear, and replacement contract is source-tested; broader shortcuts and announcements remain | Installed Orca/Wayland spoken-output evidence missing |
