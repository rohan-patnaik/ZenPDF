...
| L004 | Persistent local preferences | M0 / State | QSettings | Verified | `PreferencesTest.cpp` and `MainWindowTest.cpp` cover immutable legacy import, lock ordering, hostile/corrupt/future state, atomic save rollback, bounds, contention, concurrent snapshots, restoration, and explicit save-failure policy; [`L004_ACCEPTANCE_EVIDENCE.md`](L004_ACCEPTANCE_EVIDENCE.md) reconciles exact-package private save/relaunch, immutable versionless migration, hostile-leaf preservation, keyboard cancellation, real Wayland AT-SPI semantics and activation, and clean shutdown | N/A |
