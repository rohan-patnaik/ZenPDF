...
Reader and thumbnail workflows are not yet integrated. Task closures and `QVariant` results do not have a declared or measured byte budget, individual in-process jobs have no scheduler-enforced timeout, and a non-cooperative task can outlive timed shutdown and make final destruction wait. Engine adapters must provide their own deadlines and resource boundaries; risky helpers still require process-level isolation and tree termination. The organizer retains the qpdf adapter's separately tested 120-second polling deadline and Unix process-group cleanup rather than treating the in-process scheduler as an isolation boundary.
