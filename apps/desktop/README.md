...
The scheduler is not yet wired into reader or organizer workflows. Task closures and `QVariant` results do not have a declared or measured byte budget, individual in-process jobs have no scheduler-enforced timeout, and a non-cooperative task can outlive timed shutdown and make final destruction wait. Engine adapters must provide their own deadlines and resource boundaries; risky helpers still require process-level isolation and tree termination. The existing qpdf adapter retains its separately tested process-group behavior, while the current modal organizer runner still uses its existing QtConcurrent path.
